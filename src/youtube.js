  ...
  return gapi.client.youtubeAnalytics.reports.query({
    ...
    "endDate": "2020-01-01"
      ...
      .then(function(response) {
              ...
              // console.log(response.result.rows[0][0]);
              // console.log(response.result.rows[0][1]);
              const formattedData = [
                {
                  month: response.result.rows[0][0], views: response.result.rows[0][1],
                },
                {
                  month: response.result.rows[1][0], views: response.result.rows[1][1],
                },
                {
                  month: response.result.rows[2][0], views: response.result.rows[2][1],
                }
              ];
              console.log('data from youtube',formattedData);
              return formattedData; // this should return 'formattedData' soon 
            },
            function(err) { console.error("Execute error", err); return 'FAIL';});
}
